...
class weighted_graph {
	...
	bool has_edge(const vertex&) const; 
...
//Custom Function:
//Copies the unordered set that stores the vertices into a vector allowing for access to the vertices without the use of 
//the iterator

template <typename vertex> std::vector<vertex> weighted_graph<vertex>::get_vertices() const{
	std::vector<vertex> verticesV(vertices.begin(), vertices.end()); 
	return verticesV;
}

//Custom Function:
//Finds the position of Vertex within Vector allowing for the use of Arrays vs Maps to mark a vertex visited 
